...
func (b *BulkIndexer) Index(index string, _type string, id, op_type, ttl string, date *time.Time, data interface{}, refresh bool, consistency string) error {
	//{ "index" : { "_index" : "test", "_type" : "type1", "_id" : "1" } }
	by, err := WriteBulkBytes("index", index, _type, id, op_type, ttl, date, data, refresh, consistency)
	if err != nil {
		return err
	}
	b.bulkChannel <- by
...
func (b *BulkIndexer) Update(index string, _type string, id, op_type, ttl string, date *time.Time, data interface{}, refresh bool, consistency string) error {
	//{ "index" : { "_index" : "test", "_type" : "type1", "_id" : "1" } }
	by, err := WriteBulkBytes("update", index, _type, id, op_type, ttl, date, data, refresh, consistency)
	if err != nil {
		return err
	}
	b.bulkChannel <- by
...
func (b *BulkIndexer) UpdateWithPartialDoc(index string, _type string, id, ttl string, date *time.Time, partialDoc interface{}, upsert bool, refresh bool, consistency string) error {
	...
	return b.Update(index, _type, id, "", ttl, date, data, refresh, consistency)
}
...
// http://www.elasticsearch.org/guide/reference/api/bulk.html
func WriteBulkBytes(op string, index string, _type string, id, op_type, ttl string, date *time.Time, data interface{}, refresh bool, consistency string) ([]byte, error) {
